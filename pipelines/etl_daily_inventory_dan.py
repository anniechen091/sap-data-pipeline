...
# Google Sheets destination for the final daily inventory output.
...
def upload_inventory_to_google_sheet(inventory_df):
    ...
    staging.batch_clear([f"A1:J{max(required_rows, 2)}"])
    staging.update(
        range_name="A1:J1",
...
#Dry
sheet_id = "1VjHnVoA7WoaoA_BDpAaFvG8MS-LT3yT97fHtseSCYuA"
gid = "1718908642"
...
review_nf = pd.read_csv(csv_url, header=1)
...
pd.concat([review_dry[['Article']].drop_duplicates(), review_nf[['Article']].drop_duplicates()],axis=0).to_clipboard(index=False, header=False)
...
LEFT JOIN [dbo].[md_site] AS s ON s.Site = i.Site
LEFT JOIN [dbo].[md_article_master] AS a ON a.article_number = i.article_number
WHERE [inv_date] = (SELECT MAX (inv_date) FROM [TawaDWDB].[dbo].[inv_zinventory])
...
AND  (a.MCH BETWEEN 10600000 AND 10699999  OR
   a.MCH BETWEEN 10500000 AND 10509999)
--AND i.[article_number] = 2081673
...
storeinv = pd.read_sql(text(query), con=engine)
...
inv_final[['Unrestricted-Use Stock', 'Stock in Quality', 'On order Stock', 'storeinv']] = (
    inv_final[['Unrestricted-Use Stock', 'Stock in Quality', 'On order Stock', 'storeinv']].fillna(0)
)
...
inv_final = inv_final[['Article NoDC', 'Article', 'Site', 'Unrestricted-Use Stock', 'On order Stock', 'Stock in Quality', 'ETA', 'Walong Status', 'Tawa Status', 'storeinv']]
