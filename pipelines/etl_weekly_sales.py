...
def run_etl_weekly_sales(min_wk: int, max_wk: int,output_excel= None):
    engine = get_sql_engine()
    
    print(f"Loading weekly sales data from ZMB51 + ZSTPROMO during {min_wk} ~ {max_wk}...")
    ...
    ;WITH zstpromo AS (
        ...
        FROM    dbo.ZSTPROMO t
        JOIN    dbo.dim_Calendar c ON c.Date = t.Date
        ...
        SELECT  c.AcctWk, z.Article, z.Site,
                ...
                SUM(z.Cost)     AS Cost
        FROM    dbo.ZMB51 z
        JOIN    dbo.dim_Calendar c ON c.Date = z.Date
        WHERE   c.AcctWk BETWEEN :min_wk AND :max_wk
    ...
    FULL JOIN zstpromo p
        ...
        AND p.Site    = s.Site
    WHERE  CAST(COALESCE(p.Site, s.Site) AS INT) < 5999
 
    """
    
    table_name = os.getenv("TABLE_WEEKLY_SALES")
    df = pd.read_sql(text(query), con=engine, params={"min_wk": min_wk, "max_wk": max_wk})
    ...
    try:
        if output_excel:
            ...
            filename = f"Weekly_Sales_{min_wk}_{max_wk}_{today_str}.csv"
    ...
    #     conn.execute(text(sql), {"min_wk": min_wk, "max_wk": max_wk})

    #     print(f"ETL Weekly Sales completed for {min_wk}~{max_wk}. Uploaded to {table_name}.")
