...
def run_zmmidr_query(session, dc_code, dept_code, period_str, export_dir, filename):
    ...
    try:
        ...
        session.findById("wnd[0]/usr/radP_BUNIT").select()
...
# === 主要下載程式 ===
def download_zmmidr_BUn(EXPORT_DIR):
    ...
    for dept_name in depts:
        ...
        for dc_code in DC_CODES:
            ...
            # 檢查是否已完成
            if is_already_done("zmmidr_bun", dept_code, dc_code, time_stamp):
                print(f"✅ 已完成：{done_key}，略過")
                continue
        
            # 建立合法檔名
            filename = f"Zmmidr_bun_{done_key}.xlsx"
            success = safe_query(
                session=session,
                dept_code=dept_code,
                dc_code=dc_code,
            ...
            if success:
                record_done("zmmidr_bun", dept_code, dc_code, time_stamp)
            else:
                print(f"❌ download_zmmidr_bun 發生錯誤，中斷於：{dept_code}-{dc_code}-{time_stamp}，將於下次重新執行時繼續查詢")
                return              
...
if __name__ == "__main__":
    try:
        ...
        print(f"❌ 執行 download_zmmidr_BUn 過程中發生錯誤：{e}")